...
module type TAPS = sig
  ...
  val init : me:node -> view:view
             -> view_len:int -> xchg_len:int
             -> period:float -> io:io -> t
  (** [init node view view_len xchg_len period io]
      initializes a TAPS instance

      @param me        this node
      @param view      initial TAPS view
      ...
      @param io        network I/O implementation
   *)
