...
(** Functor building an implementation of Taps with Lwt
    given a [Node_id], [Node], gossip [View], [Taps] implementation,
...
module Make
         ...
         (Taps : P2p.S.GOSSIP with type node := Node.t
                               and type view := View.t)
         (Io : S.TAPS_IO with type nid := Node_id.t
                            and type node := Node.t
                            and type view := View.t)
       : S.TAPS with type nid := Node_id.t
                   and type node := Node.t
                   and type view := View.t
  ...
  let initiate t dst sent view =
    ...
    | (Some dst) ->
       ...
       t.view <- Gossip.merge ~view ~xview ~view_len:t.view_len
