...
(** Functor building an implementation of TAC with Lwt
    given a [Node_id], [Node], gossip [View], [Tac] implementation,
...
module Make
         ...
         (Tac : P2p.S.GOSSIP with type node := Node.t
                               and type view := View.t)
         (Io : S.TAC_IO with type nid := Node_id.t
                            and type node := Node.t
                            and type view := View.t)
       : S.TAC with type nid := Node_id.t
                   and type node := Node.t
                   and type view := View.t
